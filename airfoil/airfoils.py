...
class Airfoil:
    ...
    def _normalise_data_points(self):
        norm_factor = abs(self._x_upper[-1] - self._x_upper[0])
        self.scale_data_points(norm_factor)


    def scale_data_points(self, scale):
        self._x_upper /= scale
        self._y_upper /= scale
        self._x_lower /= scale
        self._y_lower /= scale
    ...
    def plot(self, *, show=True, save=False, line_style='-', settings={}):
        """
        ...
        ax.plot(self._x_upper, self._y_upper, line_style, color='blue')
        ax.plot(self._x_lower, self._y_lower, line_style, color='green')
